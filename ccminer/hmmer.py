...
class HMMER(object):
    ...
    def load_hmm(self):
        ...
        hmm_df = pd.read_csv(self.out+'hmmer.tab', sep='\s+', header=None,
            usecols=(0,1,3,6,7,
                     8,16,17,18,19,
                     20,21,22,24,26,28),
            names=('Hmm','Protein','tlen','qlen','Eval',
        ...
        # Add columns
        hmm_df['Acc'] = [re.sub("_[0-9]*$","",x) for x in hmm_df['Protein']]
        hmm_df['Pos'] = [int(re.sub(".*_","",x)) for x in hmm_df['Protein']]
        ...
        def covs(df_sub):
            ...
            df_sub = df_sub[['Hmm','Protein','tlen','qlen','Eval','score',
                            'start','end','Acc','Pos','Cov_seq','Cov_hmm','strand']]
            df_sub = df_sub.drop_duplicates()
            return df_sub

        hmm_df = hmm_df.groupby(['Hmm','Protein']).apply(covs)
    ...
    def read_hmm(self):
        try:        
            self.hmm_df = pd.read_csv(self.cctyper_path+'/hmmer.tab', sep='\t')
        except:
    ...
    def parse_hmm(self):
        ...
        if self.any_cas:
            ...
            self.hmm_df.drop_duplicates('Protein', inplace=True)